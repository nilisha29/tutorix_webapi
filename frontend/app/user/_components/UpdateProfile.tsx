...
//                     type="button"
//                     onClick={() => handleDismissImage(onChange)}
//                     className="absolute top-0 right-0 w-6 h-6 rounded-full bg-red-500 text-white flex items-center justify-center text-sm hover:bg-red-600"
//                   >
//                     ✕
//                   </button>
//                 )}
...
export default function UpdateUserForm({ user }: { user: any }) {
  ...
  const onSubmit = async (data: UpdateUserData) => {
    ...
    try {
      ...
      if (data.phoneNumber) formData.append("phoneNumber", data.phoneNumber);
      if (data.address) formData.append("address", data.address);
      if (data.image) formData.append("profileImage", data.image); // Changed from "image" to "profileImage"
    ...
    <div className="max-w-lg mx-auto p-6 bg-white rounded shadow mt-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <button
          type="submit"
          disabled={isSubmitting}
          className="w-full px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50 mt-4"
        >
          {isSubmitting ? "Updating..." : "Update Profile"}
        </button>
      </form>
